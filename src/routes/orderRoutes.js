...
/*--------------------------------Order Routes-------------------------------*/
router.post("/create",isAuth,orderController.createCart);//for order create (the price and total subTotal fee and many more details are calculated here )
router.post("/update/:id",isAuth, orderController.updateCart)//its just to update the order like orderDate time and mny more 
router.post("/delete/:id",isAuth,orderController.deleteCart)//it delete the order like soft delete just isDeleted make true
